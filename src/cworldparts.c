...
void CBlock_Draw(CBlock* Block)
{
    ...
    LCDBitmap *BlockImg = pd->graphics->getTableBitmap(IMG_blocks, Block->Color * 7 + Block->AnimPhase);
    pd->graphics->drawBitmap(BlockImg, Dstx, Dsty, kBitmapUnflipped);
...
{
    if(WorldParts->NeedToKillBlocks && (WorldParts->Time < pd->system->getCurrentTimeMilliseconds()))
    {
        CWorldParts_KillBlocks(WorldParts);
        WorldParts->NeedToKillBlocks = false;
        WorldParts->NeedToAddBlocks = true;
        WorldParts->Time = pd->system->getCurrentTimeMilliseconds() + 350;
        playDeleteSound();
    }

    if (WorldParts->NeedToAddBlocks && (WorldParts->Time < pd->system->getCurrentTimeMilliseconds()))
    {
    ...
    pd->graphics->drawBitmap(IMG_background, 0, 0, kBitmapUnflipped);
    int X,Y;
    for(Y=0;Y<NrOfRows;Y++)
        for(X=0;X<NrOfCols;X++)
